...
面向集团行政专员的飞书内行政海报生成工具。当前已实现员工活动 T01 四规格 Demo、文案与视觉分步确认、飞书身份和 PNG 输出；采用本地 JSON 存储及 Web 进程内异步执行。云端部署已有记录，真实用户验收仍有待办，不能视为生产就绪。

当前进度以 [HANDOFF.md](./HANDOFF.md) 开头及第 18–20 节为准；待办来源与本次简化依据见[消融记录](./docs/ABLATION_EXPERIMENT_2026-09-07.md)。较早的计划及未勾选项保留历史上下文，不代表功能均未实现。
