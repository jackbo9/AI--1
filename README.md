...
面向集团行政专员的飞书内行政海报生成工具。当前切片已实现员工活动 T01 的文案确认、视觉确认和四规格 Demo，生产化边界以 `HANDOFF.md` 为准。
...
默认 `AUTH_MODE=local`；没有配置 DeepSeek 或 Seedream 密钥时，服务端使用本地文案与品牌插画降级链路。

前端细节和工作流可以使用不调用 API、模型或 Playwright 的 Fixture 模式：

```text
http://127.0.0.1:3000/?fixture=1
```

Fixture 模式会以短延迟模拟文案、视觉和排版状态，并在同一浏览器标签刷新后恢复当前 Fixture 任务。它只用于交互演练，不代表真实生成、品牌校验或导出质量。
