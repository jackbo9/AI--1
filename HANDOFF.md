...
2026-09-13 内部试用持久化：新增 PostgreSQL/本地双仓储、私有 OSS Adapter、历史作品删除、7 天软删除与 30 天中间资产清理、只读预检优先的旧数据迁移工具。现有 API 路径保持兼容；真实 RDS/OSS 和双飞书用户验收仍需云端资源，详见 [试用持久化记录](docs/PILOT_STORAGE_2026-09-13.md)。

2026-09-13 下午茶接入：员工福利直接进入下午茶三步制作，独立食品字段/摄影规则，复用任务 API、双图选择和下载；新增场景化存储读取和重启恢复。范围、验收及当前本地端口见 [下午茶迭代记录](docs/TEA_ITERATION_2026-09-13.md)。体育锁定方案保持；本轮尚未推送或部署云端。

2026-09-13 第一版锁定：多尺寸已回退为同一母图自动裁切排版，选图后不调用图片模型或主体识别。最新行为、兼容与实际验收以[裁切锁定记录](docs/RELEASE_CROP_2026-09-13.md)为准；下方独立生成/扩图相关内容仅为历史记录。

2026-09-13 体育主视觉更新：新任务使用共享四规格坐标契约；图片请求通过右下出血边适配 16 像素倍数限制；选图后其他规格独立生成并冻结来源描述。新增 canvasTarget、Artifact.sourceBrief/actualPrompt/模型来源；渲染失败重用已生成图片。旧任务保持兼容。真实验收、费用和视觉限制见 [体育坐标修正记录](docs/SPORTS_CANVAS_2026-09-13.md)。

最后更新：2026-09-13

第一轮本地迭代：二维码可选、人物偏好直达生图确认、空白新建表单、2–4 字可编辑名单组名、名单草稿即时预览、标准/大字切换、JPG 导出已实现。说明与验收记录见 [第一轮迭代验收](docs/ITERATION_2026-09-13.md)。已配置本地 DeepSeek 文案与 gpt-image-2 图片模型，真实文案、润色、人物局部生图、正式排版和 PNG/JPG 下载通过一次完整联调；三档生图对照及飞书联调待验证。本轮尚未推送、提交 PR 或部署。以下较早记录作为历史上下文保留。

T01 标题最终规则：一级大标题最多两行，用户可在填写步骤按回车手动换行。输入 Schema 拒绝三个手动行；正式竖版、横版、Banner 和长图均按实际行盒阻止第三行，不截断、不缩字。标题换行在浏览器轻量预览与 Playwright 正式渲染中保持一致；新版本号防止复用旧三行产物。
当前竖版模板版本为 `2.1.0-t01-two-line-title`，横版、Banner 和长图为 `t01-figma-2026-09-09-v4-two-line-title`。
T01 AI 文案创建误拦修复：`POST /api/jobs` 过去无条件预检临时 `manualDocument`，导致 AI 文案路径在宣言／副标题尚待生成时用空标题组触发 y=1196 错误。现在只有 `skipCopy=true` 的手工最终文案在创建前预检；AI 路径先创建任务并生成结构化文案，用户确认生成稿时仍由 `confirm-copy` 执行完整模板预检。最终宣言必填和图片调用前发布门均未放松。
T01 决赛名单草稿修复：长图的 5 个名单组别不再阻塞文案建议、表单预览或主视觉生成。表单仅把姓名与赛区均完整的条目写入 `PosterDocument`；未完整行继续保留在本地编辑草稿，长图只渲染已完整填写的组别，无完整名单时隐藏名单模块。取消长图时名单不会随请求发送，避免隐藏空行触发 Zod 校验。
T01 长图主视觉修复：正式长图模板此前没有将 `assets.image` 写入 HTML，故填写／视觉阶段的轻量预览可显示背景，但第三步的 Playwright Artifact 丢失背景。长图现以 1080×1210、`object-fit: cover` 的顶部主视觉背景承载品牌头和标题；正文仍从 y=1210 的纯色信息区开始。额外规格模板版本升级为 `t01-figma-2026-09-10-v6-longform-hero`，不会复用旧的无背景长图 Artifact。
T01 标题预检回归修复：Figma `426:4` 将宣言纳入 y=222 的标题流后，渲染 CSS 的一级标题实际顶部为 276.5px、行高为 130px；旧预检仍使用 292px／166px，导致正常文案被误报越过 y=1196。共享布局契约现已同步为 276.5px／130px，并按完整标题与两行副标题的安全容量声明最多 6 行。新增真实任务与 38 字标题 + 40 字副标题的 Chromium 预检回归。
局域网试用服务已于 2026-09-10 切换为 `next build` 后通过 `next start` 监听 `0.0.0.0:3000`，并由 `com.ninebot.aizhihui.production` 常驻任务托管；原 `com.ninebot.aizhihui.dev` 已卸载。启动前会自动恢复因重启遗留的处理中任务；历史 Demo 任务和生成文件可通过 `npm run archive:demo -- --before=<ISO 时间>` 移入 `data/archive/`，不做永久删除。`http://10.6.4.183:3000/` 返回 307 至飞书登录入口；本次未切换阿里云 Demo。
T01 四尺寸像素回归：已扫描并以 Figma `426:4`（竖版）、`426:74`（横版）、`426:140`（Banner）、`426:156`（长图）为当前模板真源。画布为 1080×1920、1920×1080、2227×950、1080×3000，标题／副标题内容宽为 952、1188、1393、952px；宣言为新建与确认 T01 的必填字段，并作为标题组首行原样排版。箭头资产已去除错误白色画布，十字改为确定性 2px 几何线。非竖版标题分隔线现与标题区共享黑／白对比度选择，模板版本升级为 `t01-figma-2026-09-09-v3`，避免复用旧黑线 Artifact；长图内部 ID `longform_1080xAuto` 暂为历史兼容保留，但新 Artifact 固定为 3000px 高。决赛名单／赛事回顾只在提供结构化数据与图片时渲染，当前 `PosterDocument` 尚未承载这些业务字段，生产输出不会伪造人名或赛果。
T01 流程第四轮：确认主视觉并完成正式排版后，结果页改为一屏审阅布局。默认“适合屏幕”会在当前可用高度内完整展示 1080×1920 竖版，不裁切海报；右侧品牌检查、“只换主视觉”、“开始新海报”和下载操作保持可见。用户可切换“放大查看”，放大内容只在预览容器内滚动，不改变下载文件尺寸。下载继续直接使用已确认文案、已选主视觉和正式 Playwright 输出，不通过浏览器重新合成。第四轮只调整 T01 竖版结果页，未刷新其他模板。
T01 流程第三轮：每次确认视觉描述只新增一个不可变主视觉方案，不再直接排版或进入下载。方案保存图片、生成描述、来源文案版本、完整来源文案、Prompt／Brief、Provider 和模型；任务同时持久化当前选择，刷新后可继续切换。用户可以沿用同一描述再次生成，也可以修改或 AI 优化后生成；失败不会移除已有成功方案，同一次确认通过幂等键和任务状态避免重复收费。视觉阶段点击方案后，右侧直接使用受保护图片 URL 和 React/CSS 轻量合成 T01 竖版，不再请求包含内嵌字体、图片的模板 HTML，也不在切换时启动 Chromium；浏览器 Canvas 对实际 cover 裁切后的左上标题安全区进行小尺寸像素采样，自动选择黑／白标题、宣言、分隔线和公司 Logo。链接二维码在浏览器生成，上传二维码继续读取私有资源。只有显式点击“确认主视觉”后，系统才使用选中的方案进入正式排版和完整可读性检查；相关接口为 `visual-options/select` 和 `visual-options/confirm`。第三轮未改下载页适屏能力。
T01 第二轮反馈修订：默认视觉描述与图片 Provider 的系统约束统一采用品牌设计方体育赛事标准。系统会从标题／规则识别羽毛球、网球、篮球、足球、排球、乒乓球和拔河，自动选择 1–3 个器材／场地符号、推荐主色和运动瞬间；默认禁止人物、人体、手脚和面部。固定构图恢复为 `LEFT TOP = TITLE SAFE AREA`、`CENTER-RIGHT = MAIN VISUAL`（X 68%–78%、Y 48%–58%）及可延展外围背景，并声明四种裁切窗口。视觉页已移除“固定边界”提示标签；顶部流程改为连续的 1「填写与确认文案」/ 2「生成主视觉」/ 3「查看与下载」。
T01 流程第二轮：确认文案后不再展示空白视觉输入框，系统会按已确认大标题、活动类型和竖版安全区生成可编辑基础描述，明确包含主体、风格、色彩及顶部品牌／标题低细节区域。用户可直接确认，或让 AI 基于当前编辑内容优化；确认描述与确认主视觉在界面和状态文案中明确区分。基础描述、优化结果和已确认描述均绑定文案版本并写入任务，刷新、返回或重新生成时可恢复；文案版本变化会创建新的未确认描述。图片模型仍只生成主视觉，不生成 Logo、二维码、标题或活动事实。多方案保留与选定主视觉属于第三轮，尚未接入。
T01 下方信息区可读性更新：竖版的活动时间、活动地点、参与对象和参与方式不再分别选择深／浅文字。系统仍按各自真实文字行盒采样，但将 `sessions`、`audience`、`participation` 作为一个视觉组，只有同一颜色覆盖全部区域时才通过严格发布门；试用模式也统一选择覆盖区域数、最差对比度和平均对比度综合更优的一种颜色，避免相邻信息黑白交替。标题、品牌区、二维码说明和页脚仍按各自区域判断；本次未恢复遮罩或背景替换。
维护简化：本轮删除零引用手写 JSON Schema 与 `runJob` 纯转发层，生产代码净减 41 行；四规格、历史兼容与现有生成行为保持。TypeScript、ESLint、76 项测试和生产构建通过。待办来源与旧消融失效范围见第 20 节。
...
T01 文案容量更新：填写页在调用 AI 前明确提示并限制竖版主题最多 40 个字；服务端也会在创建任务前返回 `T01_TITLE_TOO_LONG`，不消耗文案或图片调用。标题使用 120px MiSans 自然换行，实测最多三行；副标题最多两行并按标题实际行盒下移 13px。文案 Prompt v1.9 将 `subtitle` 定义为实际海报副标题，要求一句、不换行、最多 40 字；模型超限会被校验并重试。确认文案不再把 150 字 `summary` 复制进副标题，副标题和补充说明分别保存。confirm-copy 顶层异常统一返回 JSON，截止时间保留原始输入。
T01 预览与底部信息修复：浏览器预览已按正式 1080 × 1920 模板等比同步三行标题、13px 标题间距和 1283px 底部信息起点，不再使用旧的一行标题宽度及互相覆盖的绝对坐标。时间、地点、参与对象和参与方式统一进入受控纵向流；有二维码时为后两组预留右侧安全列，并在正式渲染前检查文字溢出、二维码碰撞和页脚安全区。二维码说明恢复使用已确认的 `ctaLabel`。
图片 Provider 更新：新增 `openai-images` 协议适配器，可调用 OpenAI Images 兼容网关的 `/v1/images/generations`，当前已按供应商公开协议锁定 `gpt-image-2`、`1024x1792`、PNG、medium 和 Base64 响应。原 `seedream` Adapter 与环境变量回切能力保留；确认后的 Prompt 组装函数及内容未修改。新供应商 Key 未写入代码、文档或仓库，尚未在部署环境切换。
...
- A1：BrandSpec v1 运行时契约，锁定 `#FAE24C`、MiSans、
  H0 120px、H1 80px、标题最多两行及公司 Logo 左/行政标识右。
...
- 图片：一个受控主视觉资产
- 可选二维码：服务端生成的 HTTP(S) 链接二维码，或用户私有上传的二维码图片（二选一）
...
- B2 使用全幅背景、MiSans Regular/Semibold、公司 Logo 左 / 行政标识右、
  标题最多两行且支持手动换行、固定二维码区和浅色品牌降级背景，但尚未完成设计方人工视觉评审。
...
- 已接入二维码图片上传：PNG/JPEG/WebP（最大 5MB、96–8192px）由服务端 magic bytes 校验并私有保存；任务只记录受所有者保护的 `qrAssetId`，链接和上传图片二选一。当前不验证上传图片能否实际扫码。
...
- 2026-09-07 T01 标题区按 Figma `325:317` / `387:709` 同步：竖版模板版本升级为 `1.3.0-t01-multiline`，标题内容宽 920px、最多三行，副标题最多两行并与标题行盒保持 13px 间距；横版、Banner、长图容量未改变。新增 Chromium 行盒测试覆盖单/双/三/四行、空副标题与三行标题+两行副标题。该变更只同步标题区，不代表整张海报已完全同步新稿。
- 2026-09-07 T01 底部信息区继续按同一 Figma 节点同步：时间与地点拆成独立模块，使用 32px 标题、24px 正文和 4px 标题间距，模块间 16px；内容采用受控纵向流，超出底部安全区阻止导出。当前单场活动为主要验收范围，现有两场字段仍可读取并在当前容量内渲染。
...
## 21. 2026-09-04 二维码形式与上传存储

- 当前 URL 模式由服务端 `qrcode` 包即时生成 PNG Data URI；它不是上传文件。竖版二维码槽固定为 `x=864, y=1574, 144×144`；长图二维码在报名区文档流内，随截止、联系人与规则内容高度变化；横版／Banner 没有二维码槽。
...
## 20. 2026-09-07 保持功能的消融

- 基线 `d9913509683510897fd749452b7e2d260dba8d33`；删去无消费者的 `posterDocumentJsonSchema`，创建任务 API 直接异步调用 `runCopyStage`，移除无额外行为的 `runJob`。实际 Zod 校验、Provider 和状态机不变。
- 新增创建任务／重复幂等提交回归。最终 `npm run typecheck`、`npm run lint`、`npm test`（76/76）、`npm run build` 通过，`git diff --check` 通过。
- 2026-09-04 单竖版消融提出的暂停 Campaign/Artifact 双写不再适用：当前已实现四规格，必须保留来源版本、视觉族、Artifact 与历史迁移。以后收敛数据真源需独立集成验证与迁移设计。
- [本轮记录与待办来源索引](./docs/ABLATION_EXPERIMENT_2026-09-07.md)保留 KV 复用、局部调整、可选二次精修、场景契约、视觉／飞书验收、通知和生产化的原始依据。原有任务清单与评审未删除、未批量重置状态；旧清单的未勾选项不等于当前未实现。
- 本轮没有付费模型调用、线上部署、历史数据迁移或模板修改。未重跑真实飞书端到端和 PNG 视觉回归，既有 Linux 像素差异与云端验收待办不因此关闭。改动留在工作区，原有未跟踪文件保持。
# 文案交互追加调整（2026-09-13）

获取文案建议已拆为独立的 `/api/copy-suggestions`：仅需一级大标题，直接填入宣言标题与副标题供人工微调。活动事实与正式任务创建留到进入主视觉；生成失败保留原文，生成期间防重复并保护人工修改。真实浏览器联调与 4 项接口测试记录见本轮验收文档。
# 第二步交互重构（2026-09-13）

第二步改为选变量 → AI Prompt 生成 → 编辑/重新生成 → 一次两张图片 → 明确选中 → 排版。删除原“更新基础描述/AI 润色”双入口；两张独立进度与失败重试，旧单张请求继续兼容。真实任务 a643d7a1-9f2a-4597-8672-38b76d96e859 已通过描述生成、保留人工汗滴细节的重新生成、两张 gpt-image-2 图片、选图及正式 PNG/JPG 1080×1920 下载。尚未推送或部署。
# 多尺寸自动预览修复（2026-09-13）

第三步在竖版未完成时不再提前请求其他尺寸；竖版完成后自动衔接后端已有的多尺寸排版，并持续读取预览。临时请求失败自动重试，失败请求不再被错误标记为已请求；按本次成品 URL 区分请求缓存，避免换主视觉后显示旧尺寸结果。已移除“重新读取”入口。类型检查、Lint、生产构建及模拟竖版延迟/尺寸接口 503 的四规格浏览器回归通过。
