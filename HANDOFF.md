...
最后更新：2026-09-08
T01 流程第三轮：每次确认视觉描述只新增一个不可变主视觉方案，不再直接排版或进入下载。方案保存图片、生成描述、来源文案版本、完整来源文案、Prompt／Brief、Provider 和模型；任务同时持久化当前选择，刷新后可继续切换。用户可以沿用同一描述再次生成，也可以修改或 AI 优化后生成；失败不会移除已有成功方案，同一次确认通过幂等键和任务状态避免重复收费。只有显式点击“确认主视觉”后，系统才使用选中的方案进入 T01 竖版排版；相关接口为 `visual-options/select` 和 `visual-options/confirm`。第三轮未改下载页适屏能力。
