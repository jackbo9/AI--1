...
describe("activity studio UI fixture", () => {
  it("keeps locked facts while advancing through the deterministic flow", () => {
    ...
    expect(copyJob.copyDraft?.document.sessions).toEqual(input.sessions);
    expect(visualJob.status).toBe("READY_FOR_VISUAL_REVIEW");
    expect(visualJob.visualDraft?.description).toContain("不包含文字、Logo 或二维码");
    expect(readyJob.status).toBe("READY_FOR_REVIEW");
    expect(readyJob.previewUrl).toBe("/fixtures/employee-activity-poster.svg");
    expect(readyJob.versions.at(-1)?.validation.passed).toBe(true);
  });
});
