...
- [ ] 保留唯一开放场景 `employee_activity`。
- [ ] 暂时只支持 `portrait_1080x1920`。
- [ ] 在领域数据中显式记录输出 Format，避免尺寸只存在于 CSS。
- [ ] 不实现其他三个一级场景。
- [ ] 不实现横版、Banner、长图、A4 和 150 × 80mm。
- [ ] 不实现模板上传、模板选择后台和自由画布。
...
- [ ] 检查所需变量是否存在，只输出布尔状态、Provider、模型和 Base URL 域名。
- [ ] 独立调用一次 DeepSeek，确认鉴权、模型名和 JSON 输出有效。
- [ ] 独立调用一次 Seedream，确认鉴权、模型名、请求参数和图片下载有效。
- [ ] 记录响应状态、耗时、模型和错误码，不记录完整输入及密钥。
- [ ] 确认两次探针均使用脱敏测试内容。
- [ ] 探针通过后再运行完整任务，避免在 UI 调试中重复产生图片费用。

### 3.3 完善 API Provider 边界

- [ ] 浏览器不得直接访问 DeepSeek 或 Seedream。
- [ ] 所有密钥只由服务端 `src/lib/env.ts` 读取。
- [ ] DeepSeek 响应先解析为未知外部数据，再经过 Zod Schema。
- [ ] Seedream 响应先检查 `data[0].url` 或 `data[0].b64_json`。
- [ ] Seedream 返回临时 URL 时立即下载到本地存储，不能把临时 URL 当成最终产物。
- [ ] 为外部调用设置超时和有限重试。
- [ ] 401/403 归类为配置或权限错误，不自动重复请求。
- [ ] 429 和临时 5xx 最多重试一次，并使用短暂退避。
- [ ] DeepSeek 非法 JSON 或 Schema 不通过时，允许修复/重试一次。
- [ ] Seedream 失败后使用默认品牌资产，但结果必须明确标记为 `fallback`。
- [ ] 同一幂等键不得重复触发收费模型调用。

### 3.4 先完成与模板无关的交互骨架

- [ ] 将当前一次性生成流程拆成“生成文案”和“确认后生成视觉”两个阶段。
- [ ] 文案确认页允许修改可编辑字段。
- [ ] 日期、时间、地点、二维码内容和其他不可改写字段保持锁定。
- [ ] 增加“是否需要二维码”开关。
- [ ] 未开启二维码时不显示二维码输入。
- [ ] 暂时只接受二维码链接；上传图片能力等待正式模板确认后决定。
- [ ] 结果页增加“只换主视觉”操作入口。
- [ ] 只换主视觉时复用已确认的文案和模板版本。

### 3.5 保留当前可复现测试

- [ ] 保留正常、长文本、缺字段三组 Fixture。
- [ ] 增加 API Provider 的脱敏 Mock 测试。
