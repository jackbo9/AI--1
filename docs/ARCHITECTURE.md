...
2026-09-13 下午茶：工作台按场景选择各自控制器，共用 Shell、方案卡和缩放画布。`TeaJob` 与体育任务共享原子 JSON 存储但分别校验；接口按场景分派。图片请求层接受服务端编译的场景 Prompt；茶模板用同一受控 SVG 布局驱动浏览器预览及 Playwright 正式截图。未引入新基础设施。详见 [下午茶记录](TEA_ITERATION_2026-09-13.md)。
