...
2026-09-08 T01 多主视觉方案：图片生成与海报排版拆为两个独立异步动作。每次图片调用创建新的 `VisualOption`，记录不可变图片路径、确认描述、来源文案时间戳与完整 `PosterDocument`、Prompt／Brief、Provider 和模型；旧方案不覆盖。任务持久化 `selectedVisualOptionId`，切换方案不调用模型；只有确认选中方案后才创建 `VisualMaster`、竖版 Artifact 和 GenerationVersion。生成或排版失败时保留全部已成功方案，刷新从本地业务真源恢复。
