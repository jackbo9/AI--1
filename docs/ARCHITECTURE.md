...
2026-09-08 T01 多主视觉方案：图片生成与海报排版拆为两个独立异步动作。每次图片调用创建新的 `VisualOption`，记录不可变图片路径、确认描述、来源文案时间戳与完整 `PosterDocument`、Prompt／Brief、Provider 和模型；旧方案不覆盖。图片生成后通过 Chromium 对 T01 标题真实行盒执行与最终渲染相同的亮度／对比度分析，并把文字色、Logo 版本和报告保存在方案上；历史方案缺报告时在首次预览惰性补算。任务持久化 `selectedVisualOptionId`，切换方案不调用模型；预览 API 将选中图片以私有、内嵌 Data URI 注入同一 T01 HTML/CSS，并使用该方案保存的来源文案和颜色处理实时合成海报，但不创建输出文件。只有确认选中方案后才创建 `VisualMaster`、竖版 Artifact 和 GenerationVersion。生成或排版失败时保留全部已成功方案，刷新从本地业务真源恢复。
