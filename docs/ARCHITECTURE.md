...
2026-09-09 T01 四尺寸 Figma 回归：模板族以节点 `426:4`、`426:74`、`426:140`、`426:156` 为当前几何真源，画布依次为 1080×1920、1920×1080、2227×950、1080×3000。标题／副标题内容宽分别为 952、1188、1393、952px，高度随内容自然增长并以真实行盒做容量检查。`slogan`（宣言）是 T01 创建、确认和渲染的必填输入，位于标题组首行，不允许模板生成默认值。长图的 API 格式 ID 暂保留 `longform_1080xAuto` 兼容既有任务，但 v2 模板实际固定输出 3000px 高。
