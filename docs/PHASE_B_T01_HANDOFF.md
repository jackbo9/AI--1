...
- T01 标题使用 120 px MiSans 和 920 px 内容宽，支持一至三行自然换行；副标题按标题实际行盒下移 13 px。第四行及标题区越界通过真实 DOM 测量阻止导出。
- 顶部没有类别黄胶囊、眉题或其他附加标签。
...
| T01 槽位 | 数据来源 | 规则 |
|---|---|---|
| 标题 | `title` | 最多三行；超出实测容量阻止导出 |
| 顶部说明 | `subtitle` | 最多两行；为空时不输出槽位，不回退到 `summary` |
| 第一组标题 | 按场景固定文案 | `活动时间` 与 `活动地点` 独立模块；竞赛场景显示为 `比赛时间` 与 `比赛地点` |
| 第一组正文 | `sessions` | 1–2 个场次全部展示；时间与地点分别按 `场次名｜内容` 投影，不得只取第一场 |
| 第二组标题 | 固定文案 | `参与对象` |
