...
import "./job-history-actions.css";
...
export function JobHistory({ localDemo, onOpen, onNew }: { localDemo: boolean; onOpen: (id: string) => void; onNew: (scene: "01" | "02") => void }) {
  const [items, setItems] = useState<JobHistoryItem[]>([]), [cursor, setCursor] = useState<string>(), [loading, setLoading] = useState(true), [error, setError] = useState<string>(), [deleting, setDeleting] = useState<string>();
  ...
  async function remove(item: JobHistoryItem) {
    if (!window.confirm(`删除“${item.title}”的全部版本和图片？删除后将无法在历史记录中查看。`)) return;
    setDeleting(item.workId); setError(undefined);
    try {
      const response = await fetch(`/api/jobs/${item.latestJobId}`, { method: "DELETE" });
      if (!response.ok) { const payload = await response.json() as { error?: { message?: string } }; throw new Error(payload.error?.message ?? "删除失败"); }
      setItems((current) => current.filter((candidate) => candidate.workId !== item.workId));
  ...
  return <section className="ead-history-page">
    ...
    {items.length > 0 && <div className="ead-history-grid">{items.map(item => { const [status, action] = copy[item.status]; return <article className="ead-history-card" key={item.workId}><button type="button" className="ead-history-open" onClick={() => onOpen(item.latestJobId)}><span className="ead-history-cover">{item.coverUrl ? <img src={item.coverUrl} alt="" loading="lazy" /> : <i>暂无成品预览</i>}<small className={`is-${item.status}`}>{status}</small></span><span className="ead-history-meta"><small>{item.scene === "employee-afternoon-tea" ? "员工福利 · 下午茶" : "员工活动 · 体育赛事"}</small><b>{item.title}</b>{item.subtitle && <span>{item.subtitle}</span>}<footer><time>{formatTime(item.updatedAt)}</time>{item.versionCount > 1 && <i>{item.versionCount} 个版本</i>}</footer><strong>{action} →</strong></span></button><button type="button" className="ead-history-delete" disabled={deleting === item.workId} onClick={() => void remove(item)}>{deleting === item.workId ? "删除中…" : "删除"}</button></article>; })}</div>}
