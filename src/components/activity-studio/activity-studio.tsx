...
export function ActivityStudio({ identity, fixtureMode = false }: ActivityStudioProps) {
  ...
  >
    <div className="ead-activity-content">
      ...
      {restoring ? <LoadingCard title="正在恢复任务" detail="正在读取本地预览状态…" /> : stage === 1 && <StepOne form={form} qrMode={qrMode} qrUploadPending={qrUploadPending} onField={updateForm} onQrUrl={updateQrUrl} onSession={updateSession} onQrMode={changeQrMode} onQrUpload={uploadQr} onClearQrAsset={clearQrAsset} onSubmit={submit} pending={pendingAction === "submit"} />}
      {!restoring && stage === 2 && <StepTwo form={form} review={copyReview} job={job} onChange={setCopyReview} onBack={() => setStage(1)} onConfirm={confirmCopy} pending={pendingAction === "copy"} />}
      {!restoring && stage === 3 && <StepThree job={job} visualIdea={visualIdea} visualDescription={visualDescription} onIdea={setVisualIdea} onDescription={changeVisualDescription} onRefine={refineVisual} onBack={() => setStage(2)} onConfirm={confirmVisual} pendingRefine={pendingAction === "refine"} pendingConfirm={pendingAction === "visual"} />}
