...
export function ActivityStudio({ identity, fixtureMode = false }: ActivityStudioProps) {
  ...
  >
    <div className="ead-activity-content">
      ...
      {!restoring && stage === 3 && <StepThree job={job} visualDescription={visualDescription} onDescription={changeVisualDescription} onRefine={refineVisual} onBack={() => setStage(1)} onConfirm={confirmVisual} pendingRefine={pendingAction === "refine"} pendingConfirm={pendingAction === "visual"} />}
