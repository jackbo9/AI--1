...
function DraftPreview({ form, copy, hasQr }: { form: FormState; copy?: PosterDocument; hasQr: boolean }) {
  ...
  return <div className="t01-preview"><img className="t01-preview-background" src="/brand/employee-activity-fallback.svg" alt="" /><header className="t01-preview-header"><img src="/brand/company-logo.svg" alt="九号公司" /><img src="/brand/administration-mark.svg" alt="行政" /></header><section className="t01-preview-title"><h3>{copy?.title || form.activityName || "活动主题"}</h3><p>{copy ? copy.subtitle : form.supplement || "活动说明将在确认后显示"}</p></section><section className="t01-preview-info t01-preview-sessions"><div className="t01-preview-session-block"><h4>活动时间</h4>{sessions.map((session, index) => <p key={`time-${index}-${session.date}`}>{session.date || "日期"} {session.time || "时间"}</p>)}</div><div className="t01-preview-session-block"><h4>活动地点</h4>{sessions.map((session, index) => <p key={`location-${index}-${session.date}`}>{session.location || "地点"}</p>)}</div></section><section className="t01-preview-info t01-preview-audience"><h4>参与对象</h4><p>{form.audience || "参与对象"}</p></section><section className={`t01-preview-info t01-preview-participation ${hasQr ? "has-qr" : ""}`}><h4>参与方式</h4>{participation.slice(0, 2).map((step, index) => <p key={`${index}-${step}`}>{step}</p>)}</section>{hasQr && <aside className="t01-preview-qr"><b>QR</b><span>扫码报名</span></aside>}<footer className="t01-preview-footer"><span>九号行政</span><span>员工活动</span></footer></div>;
}
