...
export function LivePreview({ form, copy, hasQr, job, stage, statusLabel, fixtureMode, activeRenderTarget, onRenderTarget }: { form: FormState; copy?: PosterDocument; hasQr: boolean; job?: ActivityJob; stage: Stage; statusLabel: string; fixtureMode: boolean; activeRenderTarget: RenderTargetId; onRenderTarget: (target: RenderTargetId) => void }) {
  ...
  const selectedVisualOption = fixtureMode
    ? undefined
    : job?.visualOptions?.find(
  ...
  const previewKey = useMemo(
    () =>
      JSON.stringify({
        document,
        format: activeRenderTarget,
        jobId: selectedVisualOption ? job?.id : undefined,
        visualOptionId: selectedVisualOption?.id
      }),
    [activeRenderTarget, document, job?.id, selectedVisualOption]
  );
  useEffect(() => {
  ...
  const portraitResult =
    stage === 3 && activeRenderTarget === "portrait_1080x1920"
      ? job?.previewUrl
      : undefined;
  return <aside className="ead-preview"><div className="ead-preview-head"><header><b>实时预览</b><span>{fixtureMode ? "Fixture 演练" : stage === 2 && selectedVisualNumber > 0 ? `方案 ${selectedVisualNumber} · 实时合成` : statusLabel}</span></header><div className="ead-size-tabs">{previewTargets.filter(([target]) => form.renderTargets.includes(target)).map(([target, name]) => <button type="button" key={target} className={activeRenderTarget === target ? "is-selected" : ""} onClick={() => onRenderTarget(target)}>{name}</button>)}</div><small className="ead-size-note">与最终模板共用同一份 HTML/CSS</small></div><div className="ead-canvas-wrap"><div className={`ead-poster-frame is-${activeRenderTarget}`}>{portraitResult ? <img className="ead-generated-poster" src={portraitResult} alt="生成的员工活动海报" /> : html ? <iframe title="T01 海报预览" className={`t01-preview-frame is-${activeRenderTarget}`} srcDoc={html} sandbox="" /> : <span className="t01-preview-loading">正在同步模板…</span>}</div></div>{stage === 3 && portraitResult && <div className="ead-exportbar"><a href={portraitResult} download={fixtureMode ? "employee-activity-fixture.svg" : "employee-activity-t01.png"}>{fixtureMode ? "下载演练稿" : version?.validation.exportAllowed === false ? "下载不可用" : version?.validation.passed ? "下载 PNG" : "下载风险结果"}</a><small>{fixtureMode ? "Fixture 成品 · 不代表真实生成质量" : version?.validation.passed ? "质量检查通过" : "图文对比度未通过，可重新生成主视觉"}</small></div>}</aside>;
}
