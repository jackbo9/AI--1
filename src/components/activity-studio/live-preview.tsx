...
export function LivePreview({ form, copy, hasQr, job, stage, statusLabel, fixtureMode, activeRenderTarget, onRenderTarget }: { form: FormState; copy?: PosterDocument; hasQr: boolean; job?: ActivityJob; stage: Stage; statusLabel: string; fixtureMode: boolean; activeRenderTarget: RenderTargetId; onRenderTarget: (target: RenderTargetId) => void }) {
  ...
  const selectedVisualNumber = selectedVisualOption
    ...
    : 0;
  const displayTarget =
    stage === 2 ? "portrait_1080x1920" : activeRenderTarget;
  ...
  const visibleTargets =
    stage === 2
      ? previewTargets.filter(([target]) => target === "portrait_1080x1920")
      : previewTargets.filter(([target]) => form.renderTargets.includes(target));
  return <aside className="ead-preview"><div className="ead-preview-head"><header><b>实时预览</b><span>{fixtureMode ? "Fixture 演练" : stage === 2 && selectedVisualNumber > 0 ? `方案 ${selectedVisualNumber} · 轻量预览` : statusLabel}</span></header><div className={`ead-size-tabs ${visibleTargets.length === 1 ? "is-single" : ""}`}>{visibleTargets.map(([target, name]) => <button type="button" key={target} className={displayTarget === target ? "is-selected" : ""} onClick={() => onRenderTarget(target)}>{name}</button>)}</div><small className="ead-size-note">{stage === 2 ? "自动适配黑/白文字，确认后执行正式排版" : "与最终模板共用同一份 HTML/CSS"}</small></div><div className="ead-canvas-wrap"><div className={`ead-poster-frame is-${displayTarget}`}>{stage === 2 ? selectedVisualOption ? <LightweightT01Preview document={selectedVisualOption.sourceDocument} imageUrl={selectedVisualOption.previewUrl} optionId={selectedVisualOption.id} /> : <span className="t01-preview-loading">生成并选择主视觉后，在这里查看完整海报</span> : portraitResult ? <img className="ead-generated-poster" src={portraitResult} alt="生成的员工活动海报" /> : html ? <iframe title="T01 海报预览" className={`t01-preview-frame is-${displayTarget}`} srcDoc={html} sandbox="" /> : <span className="t01-preview-loading">正在同步模板…</span>}</div></div>{stage === 3 && portraitResult && <div className="ead-exportbar"><a href={portraitResult} download={fixtureMode ? "employee-activity-fixture.svg" : "employee-activity-t01.png"}>{fixtureMode ? "下载演练稿" : version?.validation.exportAllowed === false ? "下载不可用" : version?.validation.passed ? "下载 PNG" : "下载风险结果"}</a><small>{fixtureMode ? "Fixture 成品 · 不代表真实生成质量" : version?.validation.passed ? "质量检查通过" : "图文对比度未通过，可重新生成主视觉"}</small></div>}</aside>;
}
