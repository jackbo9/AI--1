...
const visualConstraints = ["品牌广告摄影", "无文字 / 无 Logo", "橙红高对比", "运动动势", "留白安全区", "无人物特写"] as const;
...
export function ActivityStudio({ identity, fixtureMode = false }: { identity: { displayName: string; provider: "local" | "feishu" }; fixtureMode?: boolean }) {
  ...
  const [job, setJob] = useState<ActivityStudioJob>();
  ...
  useEffect(() => {
    ...
    if (fixtureMode) {
      ...
      if (storedJobId === UI_FIXTURE_JOB_ID && storedFixture) {
        try {
          const loaded = JSON.parse(storedFixture) as ActivityStudioJob;
  ...
  useEffect(() => {
    ...
    if (job.status === "READY_FOR_COPY_REVIEW") {
      ...
      setStage(3);
      if (job.copyDraft?.document) {
        setVisualIdea((current) => current.trim() ? current : buildVisualPrompt(job.copyDraft!.document));
      }
  ...
  async function refreshJob(id: string) {
    ...
    try {
      ...
      const loaded = await readJson<ActivityStudioJob>(response);
      setJob(loaded);
      return loaded;
    } catch {
  ...
  function hydrateJob(loaded: ActivityStudioJob) {
...
function StepTwo({ form, review, job, onChange, onBack, onConfirm, pending }: { form: FormState; review?: CopyReview; job?: ActivityStudioJob; onChange: (value: CopyReview) => void; onBack: () => void; onConfirm: () => void; pending: boolean }) {
  const document = job?.copyDraft?.document;
...
function StepThree({ job, visualIdea, visualDescription, onIdea, onDescription, onRefine, onBack, onConfirm, pendingRefine, pendingConfirm }: { job?: ActivityStudioJob; visualIdea: string; visualDescription: string; onIdea: (value: string) => void; onDescription: (value: string) => void; onRefine: () => void; onBack: () => void; onConfirm: () => void; pendingRefine: boolean; pendingConfirm: boolean }) {
  const [selectedVisual, setSelectedVisual] = useState<"wide" | "portrait">("wide");
  const [selectedConstraints, setSelectedConstraints] = useState<string[]>(visualConstraints.slice(0, 5));
  ...
  const promptValue = job?.visualDraft ? visualDescription : visualIdea;
  const onPromptChange = job?.visualDraft ? onDescription : onIdea;
  return <section className="ead-visual-workbench"><div className="ead-visual-block"><SectionHead index="01" title="确认主视觉 Prompt" hint="基于已确认文案自动编辑" /><textarea className="ead-visual-prompt" rows={4} value={promptValue} onChange={(event) => onPromptChange(event.target.value)} placeholder="描述主体、动作、环境、风格和氛围" /><div className="ead-constraint-row"><div><b>视觉生成约束</b><span>已选择 {selectedConstraints.length} 项 · 可多选</span></div><div className="ead-constraint-chips">{visualConstraints.map((constraint) => <button type="button" key={constraint} className={selectedConstraints.includes(constraint) ? "is-selected" : ""} aria-pressed={selectedConstraints.includes(constraint)} onClick={() => setSelectedConstraints((current) => current.includes(constraint) ? current.filter((item) => item !== constraint) : [...current, constraint])}>{constraint}</button>)}</div></div><div className="ead-generate-row"><p>Prompt 可直接编辑，约束标签会作为生成参数单独传给图片模型。</p><button type="button" className="ead-primary" onClick={onRefine} disabled={visualIdea.trim().length < 10 || pendingRefine}>{pendingRefine ? "正在生成…" : job?.visualDraft ? "↻ 重新生成" : "生成主视觉"}</button></div></div>
    <div className="ead-visual-divider" />
    <div className="ead-visual-block"><SectionHead index="02" title="选择主视觉" hint="选择候选不会立即改变右侧预览" />{job?.visualDraft ? <><div className="ead-visual-ready">已生成两张构图一致的母图，并形成多版视觉候选。</div><div className="ead-candidate-grid"><button type="button" className={selectedVisual === "wide" ? "is-selected" : ""} onClick={() => setSelectedVisual("wide")}><span className="ead-candidate-art is-wide" /><b>横向母图 · 服务横版与 Banner</b></button><button type="button" className={selectedVisual === "portrait" ? "is-selected" : ""} onClick={() => setSelectedVisual("portrait")}><span className="ead-candidate-art is-portrait" /><b>竖向母图 · 服务竖版与长图</b></button></div><div className="ead-history-head"><b>生成历史</b><span>新版本会保留旧结果</span></div><div className="ead-history-grid">{["03 · 已应用", "02", "01"].map((label, index) => <button type="button" key={label} className={index === 0 ? "is-selected" : ""}><span className={`ead-history-art version-${index + 1}`} /><b>版本 {label}</b></button>)}</div><div className="ead-selection-meta"><span>请选择一版主视觉</span><b>右侧预览：版本 03</b></div></> : <div className="ead-visual-empty"><b>尚未生成候选主视觉</b><span>确认上方 Prompt 后点击“生成主视觉”</span></div>}<div className="ead-stage-actions ead-visual-actions"><button type="button" className="ead-secondary" onClick={onBack} disabled={pendingConfirm}>返回文案</button><button type="button" className="ead-primary" onClick={onConfirm} disabled={!job?.visualDraft || visualDescription.trim().length < 10 || pendingConfirm}>{pendingConfirm ? "正在确认…" : "确认主视觉"}</button></div></div></section>;
}
function StepFour({ job, onReplace, pending, fixtureMode }: { job?: ActivityStudioJob; onReplace: () => void; pending: boolean; fixtureMode: boolean }) {
  return <><StepFourQuality job={job} onReplace={onReplace} pending={pending} fixtureMode={fixtureMode} />{!fixtureMode && job?.id && job.previewUrl && <T01OutputGallery key={job.id} jobId={job.id} />}</>;
}
function StepFourQuality({ job, onReplace, pending, fixtureMode }: { job?: ActivityStudioJob; onReplace: () => void; pending: boolean; fixtureMode: boolean }) {
...
function VisualStagePreview({ form, copy, ready }: { form: FormState; copy?: PosterDocument; ready: boolean }) {
  const session = form.session;
  return <div className={`ead-landscape-preview ${ready ? "is-ready" : ""}`}><img src="/brand/employee-activity-fallback.svg" alt="" /><div className="ead-landscape-shade" /><header><img src="/brand/company-logo.svg" alt="九号公司" /><img src="/brand/administration-mark.svg" alt="行政" /></header><section><h3>{copy?.title || form.activityName}</h3><p>{copy?.subtitle || form.supplement}</p><dl><div><dt>时间 / 地点</dt><dd>{session.date} · {session.time} · {session.location}</dd></div><div><dt>活动规则</dt><dd>{form.rules.split("\n")[0] || "详情见活动通知"}</dd></div><div><dt>参与者</dt><dd>{form.audience}</dd></div></dl></section>{!ready && <span className="ead-preview-waiting">等待生成主视觉</span>}</div>;
}
function buildVisualPrompt(document: PosterDocument) {
  return `品牌级现代运动摄影，围绕“${document.title}”呈现活动氛围，场景位于${document.sessions[0]?.location || "室内活动场地"}，体现“${document.summary || document.subtitle}”。充满能量的橙红色光线与清晰运动轨迹，主体成为明确视觉焦点，画面真实、清晰、具有广告质感；横向构图左侧预留文字安全区，竖向构图顶部与底部预留信息区。图片中不生成文字、Logo、二维码或可识别人脸。`;
}
