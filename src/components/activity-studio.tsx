...
export function ActivityStudio({ identity }: { identity: { displayName: string; provider: "local" | "feishu" } }) {
  ...
  function hydrateJob(loaded: Job) {
    ...
    if (document) setForm((current) => ({ ...current, activityName: document.title, session: document.sessions[0] ?? current.session, secondSession: document.sessions[1] ? { date: document.sessions[1].date, time: document.sessions[1].time, location: document.sessions[1].location } : undefined, audience: document.audience, supplement: document.summary, rules: document.rules ?? "", prize: document.prize ?? "", qrUrl: document.qrPayload, qrAssetId: document.qrAssetId, qrAssetPreviewUrl: document.qrAssetId ? `/api/uploads/qr/${document.qrAssetId}` : undefined, qrAssetName: document.qrAssetId ? "已上传二维码图片" : "", contact: document.contact, deadline: document.deadline ?? "" }));
  ...
  return <div id="employee-activity-demo" lang="zh-CN"><div className="ead-window">
    ...
    <main className="ead-workspace"><div className="ead-activity-content"><div className="ead-title-row"><div><em>员工活动</em><h1>制作一套活动海报</h1><p>一份内容，先确认文案，再生成主视觉</p></div><small>本地演示 · 状态随任务恢复</small></div>{restoring ? <LoadingCard title="正在恢复任务" detail="正在读取本地预览状态…" /> : stage === 1 && <StepOne form={form} qrMode={qrMode} qrUploadPending={qrUploadPending} onField={updateForm} onQrUrl={updateQrUrl} onSession={updateSession} onAddSecond={() => setForm((current) => ({ ...current, secondSession: { date: "", time: "", location: "" } }))} onRemoveSecond={() => setForm((current) => ({ ...current, secondSession: undefined }))} onQrMode={(mode) => { setQrMode(mode); if (mode === "none") setForm((current) => ({ ...current, qrUrl: "", qrAssetId: "", qrAssetPreviewUrl: undefined, qrAssetName: "" })); }} onQrUpload={uploadQr} onClearQrAsset={() => setForm((current) => ({ ...current, qrAssetId: "", qrAssetPreviewUrl: undefined, qrAssetName: "" }))} onSubmit={submit} pending={pendingAction === "submit"} />}{!restoring && stage === 2 && <StepTwo form={form} review={copyReview} job={job} onChange={setCopyReview} onBack={() => setStage(1)} onConfirm={confirmCopy} pending={pendingAction === "copy"} />}{!restoring && stage === 3 && <StepThree job={job} visualIdea={visualIdea} visualDescription={visualDescription} onIdea={setVisualIdea} onDescription={(value) => { visualDescriptionRef.current = value; setVisualDescription(value); }} onRefine={refineVisual} onBack={() => setStage(2)} onConfirm={confirmVisual} pendingRefine={pendingAction === "refine"} pendingConfirm={pendingAction === "visual"} />}{!restoring && stage === 4 && <StepFour job={job} onReplace={replaceVisual} onRestart={startNewPoster} pending={pendingAction === "replace"} />}{(error || job?.error) && <p className="ead-error">{error ?? `${job?.error?.message}（${job?.error?.code}）`}</p>}</div></main>
      <aside className="ead-preview"><div className="ead-preview-head"><header><b>实时预览</b><span>{statusLabel}</span></header><div className="ead-size-tabs"><button type="button" className="is-selected">1080 × 1920</button></div><small className="ead-size-note">排版导出中可生成横版、Banner 和长图</small></div><div className="ead-canvas-wrap"><div className="ead-poster-frame is-portrait">{job?.previewUrl ? <img className="ead-generated-poster" src={job.previewUrl} alt="生成的员工活动海报" /> : <Preview form={form} copy={previewCopy} hasQr={qrMode === "add" && (Boolean(form.qrUrl) || Boolean(form.qrAssetId))} />}</div></div>{stage === 4 && job?.previewUrl && <div className="ead-exportbar"><a href={job.previewUrl} download="employee-activity-t01.png">{version?.validation.exportAllowed === false ? "下载不可用" : version?.validation.passed ? "下载 PNG" : "下载试用稿"}</a><small>当前尺寸：1080 × 1920 · {version?.validation.strategy === "trial" ? "试用策略" : "严格策略"}</small></div>}</aside>
    </div></div></div>;
}
...
function StepFour({ job, onReplace, onRestart, pending }: { job?: Job; onReplace: () => void; onRestart: () => void; pending: boolean }) {
  return <><StepFourQuality job={job} onReplace={onReplace} onRestart={onRestart} pending={pending} />{job?.id && job.previewUrl && <T01OutputGallery key={job.id} jobId={job.id} />}</>;
}
function StepFourQuality({ job, onReplace, onRestart, pending }: { job?: Job; onReplace: () => void; onRestart: () => void; pending: boolean }) {
